...
@stable default_mode = "disable" begin
    ...
    include("QDArchive.jl")  # ASOUL asoul-qd-v1: MAP-Elites archive
...
# ── ASOUL asoul-qd-v1: MAP-Elites archive for search-time QD ──
...
function _main_search_loop!(
    ...
    # ── ASOUL asoul-qd-v1: per-output MAP-Elites archive allocation ──
    ...
    while sum(state.cycles_remaining) > 0
        ...
        if population_ready
            ...
            # ── ASOUL asoul-qd-v1: parallel QD archive update ──
            ...
            if options.hof_migration && length(dominating) > 0
                # ── ASOUL asoul-qd-v1: diversity-biased migration source ──
