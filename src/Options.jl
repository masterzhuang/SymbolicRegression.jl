...
# HallOfFame / PopMember / Complexity, all of which load later). The
# parsing contract is identical to QDArchive.qd_options_from_env;
# the two copies must stay in sync, and the authoritative spec is
# documented in
...
@unstable @save_kwargs DEFAULT_OPTIONS function Options(;
    ...
    # ── ASOUL QD archive kwargs (asoul-qd-v1-on-1.11) ───────────────
    # Defaults come from the env-var bridge (_resolve_qd_options_from_env
    # above) so that Python can flip them without a PySR kwarg-whitelist
    ...
    options = Options{
        ...
        use_recorder,
        # ── ASOUL QD archive (asoul-qd-v1-on-1.11) ──
        use_qd_archive,
        qd_migration,
        qd_migration_k,
        qd_max_cells,
